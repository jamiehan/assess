...
//	 *      question)
	...
	public void deleteQuestionTestByTestId(int testId) {
		// TODO Auto-generated method stub
		try {
			questionMapper.deleteQuestionTestByTestId(testId);
			questionMapper.deleteQuestionByTestId(testId);
		}catch (Exception e) {
			// TODO Auto-generated catch block

			e.printStackTrace();
			throw new RuntimeException("删除导入试卷错误，请检查！" + e.getMessage());
		}
	...
	public void uploadQuestions(String filePath, String username, int fieldId) {
		...
		try {
			...
			if("xls".equals(filetype.toLowerCase()) || "xlsx".equals(filetype.toLowerCase())) {
				...
				parseWordService.parseWord(filePath,pointMap);
...
//				}
	...
	public void uploadQuestions2(Question question2, String username, int fieldId) {
		...
		List<Integer> pointList2 = question2.getPointList();
		...
		filePath = System.getProperty("catalina.base") + strPath.replace(',', File.separatorChar) + File.separatorChar
				+ filePath;
		question2.setFilePath(filePath);
		Map<String, KnowledgePoint> pointMap = this.getKnowledgePointMapByFieldId(fieldId, null);
		int index = 2;
		try {
			String filetype = filePath.substring(filePath.lastIndexOf(".") + 1);
			if("xls".equals(filetype.toLowerCase()) || "xlsx".equals(filetype.toLowerCase())) {
				...
				parseWordService.parseWord(question2,pointMap);
...
//				}
	...
	public void uploadQuestionsAnswer(Question question2, String username, int testId) {
		...
		try {
			...
			if("doc".equals(filetype.toLowerCase()) || "docx".equals(filetype.toLowerCase())) {
				parseWordService.parseWordAnswer(question2,testId);
			}
		} catch (Exception e) {
			// TODO Auto-generated catch block
	...
	public void updateQuestion(Question question, List<QuestionTag> questionTagList) {
		...
		try {
			System.out.printf("questionTagList =============== " + questionTagList);
			System.out.printf("questionTagList 》》》》》》》》》》 " + question.getAnswer());
	...
	public Question getQuestionDetail(int questionId,int userId) {
		...
		try {
			...
			Gson gson = new Gson();
			question.setQuestionContent(gson.fromJson(question.getContent(), QuestionContent.class));
			if(question.getQuestionContent() != null){
				question.getQuestionContent().getChoiceList();
	...
	public List<QuestionTest> getQuestionTestList(int start, int end) {
		return questionMapper.getQuestionTestList(start,end);
	}

	@Override
	public List<QuestionTest> getQuestionTestPageList(Page<Question> pageModel, QuestionFilter qf) {
		return questionMapper.getQuestionTestPageList(qf,pageModel);
	}
