...
//	request.setAttribute("leftMenuId",list[3]);
	...
	request.setAttribute("leftMenuId","cms-expert-list");
%>
...
<body>
<header>
	<span style="display:none;" id="expert-role-val"><%=list[1]%></span>
	<div class="container">
		<div class="row">
		...
		<div class="row">
			...
			<div class="col-xs-10" id="right-content">
				<div class="page-header">
					<h1><i class="fa fa-bar-chart-o"></i>专家库</h1>
				</div>
				<div class="page-content">
					<div id="dep-list">
						<div class="form-line page-content">
							<span class="form-label">专家库类型：</span>
							<%--<select id="cat_id">--%>
						...
						<div class="table-controller">
							<div class="btn-group table-controller-item" style="float:left">
									...
									<a href="<%=basePath%>admin/expert/expert_add">添加专家库</a>
									<%--<i class="fa fa-plus-square"></i>--%>
									<%--<a href="<%=basePath%>admin/expert/expert_cat_add">添加资料库分类</a>--%>
						...
						<table class="table-striped table">
							...
							<tbody>
							<c:forEach items="${cmsExpertList }" var="items">
								<tr><td style="display:none;">
									...
									<td>${items.catName }</td>
									<td>
										<a href="<%=list[1]%>/expert/cms-expert-show/${items.id }" target="_blank" title="预  览">${items.title }</a>
									...
									<td>
										<c:choose>
											<c:when test="${items.creator == sessionScope.SPRING_SECURITY_CONTEXT.authentication.principal.username}">
												<a class="change-property btn-sm btn-info" href="<%=basePath%>admin/expert/expert_update/${items.id}"><i class="ace-icon fa fa-pencil bigger-120" id="${items.id}"></i></a>
												<a class="delete-expert-btn btn-sm btn-danger" id="${items.id}"><i class="ace-icon fa fa-trash-o bigger-120"></i></a>
...
<script>
    $(function() {
        ...
        $("#cat_id").change(function() {
			...
			window.location.href = "<%=request.getContextPath()%>/admin/cms/cms-expert-list?catId=" + catId;
        });

        $(".delete-expert-btn").click(function(){
            ...
            if(confirm("确定要删除吗？删除后无法恢复")){
                jQuery.ajax({
                    ...
                    type : "GET",
                    url : '/admin/cms/delete-cmsExpert/' + paper_id,
