...
import com.examstack.portal.service.CmsRulesService;
...
public class RulesPage {
	...
	@Autowired
	private CmsRulesService cmsRulesService;
	...
	public String rulesListPage(Model model, HttpServletRequest request, @RequestParam(value="page",required=false,defaultValue="1") int page, String catId) {
		...
		String pageStr = PagingUtil.getPagelink(page,
				pageModel.getTotalPage(),"", "/" + Constants.CONTEXT_PATH_PORTAL + "/rules-list");
		...
		model.addAttribute("rulesList", cmsRulesList);
		model.addAttribute("pageStr", pageStr);
		return "rules-list";
	}

    @RequestMapping(value = "/rules-show/{id}", method = RequestMethod.GET)
